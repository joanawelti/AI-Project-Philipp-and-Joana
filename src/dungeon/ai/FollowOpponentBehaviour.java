...
public class FollowOpponentBehaviour implements Behaviour {
	/**
	 * Constructor
	 * 
	 * @param creature The creature
	 */
	public FollowOpponentBehaviour(Creature creature)
	{
		fCreature = creature;
	...
	boolean move(Game game) {
		...
		int cretureindex=game.getCreatures().size()-1;
		if (targetPosition == null) {
			targetPosition = game.getCreatures().elementAt(cretureindex).getLocation();
		}

		
		//if he's moved too far, or we had no path anyway
		if (targetPosition.distance(game.getCreatures().elementAt(cretureindex).getLocation()) > 4
				|| wayPoints == null) {
			
			wayPoints = followAction.findPath(game, game.getCreatures().elementAt(cretureindex).getLocation());
			targetPosition = game.getCreatures().elementAt(cretureindex).getLocation();
