...
public class FQLearnerGenData extends BehaviourWithPathfindingAStar {
	...
	public FQLearnerGenData(Creature creature) {
		super(creature);
	}
	...
	public boolean onTick(Game game) {
		...
		case 0:
			...
			if (fileSize100()&&startCheck) {
				FQ_learner.getInstance().transitionList = readFile();
		...
		case 1:
			// do the training
			FQ_learner.getInstance().FQwithGraphics();
			FQ_learner.runningstate = 2;
			break;
		case 2:
			// now ready to act using what we've learned
			int newAction= FQ_learner.getInstance().getBestAction(
...
//		if(FQ_learner.dataIterYpos>2.4&& FQ_learner.dataIterYpos<2.6)
//				System.out.println("pos"+fCreature.getLocation());
	...
	private void generateTransitions(Game game) {
		...
		if (tick == 0) {
			...
			if (FQ_learner.dataIterXpos > 2.55) {
				if (FQ_learner.dataIterEnergy < 0) {
					FQ_learner.dataIterXpos -= distIterStepX;
					FQ_learner.dataIterEnergy = 100;
				}
				game.getCreatures().elementAt(1).placeAt(
						new Point2D.Double(27.5, 22.5), game);
				game.getCreatures().elementAt(1).setCurrentHealth(100);
				fCreature.setCurrentEnergy((int) FQ_learner.dataIterEnergy);
				fCreature.setCurrentHealth((int) 100);
				fCreature.placeAt(new Point2D.Double(FQ_learner.dataIterXpos,
						FQ_learner.dataIterYpos), game);
				newState = getRealTimeState(game);

				if (FQ_learner.dataIterAttack) {
					oldAction = 1;
					targetPosition = null;
					wayPoints = null;
					FQ_learner.dataIterAttack = false;
				} else {
					oldAction = 0;
					targetPosition = null;
					wayPoints = null;
					FQ_learner.dataIterAttack = true;
					//inner loop is for energy, decreasing
					if (FQ_learner.dataIterEnergy > -1) {

						FQ_learner.dataIterEnergy -= energyIterStep;
					}
				}

			} else
			// outer loop for distance, going up the left side
			if (FQ_learner.dataIterYpos > 2) {
				if (FQ_learner.dataIterEnergy < 0) {
					FQ_learner.dataIterYpos -= distIterStepY;
					FQ_learner.dataIterEnergy = 100;
				}
				game.getCreatures().elementAt(1).placeAt(
						new Point2D.Double(27.5, 22.5), game);
		...
		try {
			...
			while (reader.ready()) {
				...
				if (!values.contains("New")) {
					...
					Transition transIn = new Transition(new OgreState(Double
							.valueOf(valuesArray[0]), Double
							.valueOf(valuesArray[1]), Double
							.valueOf(valuesArray[2]), Double
							.valueOf(valuesArray[3])), Integer
							.parseInt(valuesArray[4]), new OgreState(Double
							.valueOf(valuesArray[5]), Double
							.valueOf(valuesArray[6]), Double
							.valueOf(valuesArray[7]), Double
							.valueOf(valuesArray[8])));
